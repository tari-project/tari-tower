...
class Xn extends Zn {
  constructor(n, s) {
    ...
    let S = !1;
    const x = new $(), b = new $(), C = new $(), P = new $(), N = new $(), _ = new $(), p = new $(), g = new $(), m = new $(), r = [], l = {};
    function y() {
    ...
    function Le(o) {
      e.object.isPerspectiveCamera ? e.scale /= o : e.object.isOrthographicCamera ? (e.object.zoom = Math.max(e.minZoom, Math.min(e.maxZoom, e.object.zoom * o)), e.object.updateProjectionMatrix(), S = !0) : (console.warn("WARNING: OrbitControls.js encountered an unknown camera type - dolly/zoom disabled."), e.enableZoom = !1);
    }
    function de(o) {
      e.object.isPerspectiveCamera ? e.scale *= o : e.object.isOrthographicCamera ? (e.object.zoom = Math.max(e.minZoom, Math.min(e.maxZoom, e.object.zoom / o)), e.object.updateProjectionMatrix(), S = !0) : (console.warn("WARNING: OrbitControls.js encountered an unknown camera type - dolly/zoom disabled."), e.enableZoom = !1);
    }
    ...
    function Cn(o) {
      b.set(o.clientX, o.clientY), C.subVectors(b, x).multiplyScalar(e.rotateSpeed);
      const E = e.domElement;
      B(2 * Math.PI * C.x / E.clientHeight), H(2 * Math.PI * C.y / E.clientHeight), x.copy(b), e.update();
    }
    function En(o) {
    ...
    function zt(o) {
      ...
      C.subVectors(b, x).multiplyScalar(e.rotateSpeed);
      const E = e.domElement;
      B(2 * Math.PI * C.x / E.clientHeight), H(2 * Math.PI * C.y / E.clientHeight), x.copy(b);
    }
    function It(o) {
      if (r.length === 1)
...
const ao = () => {
  function i() {
    oe = Array.from(
      { length: j },
      (f, h) => Array.from({ length: j }, (w, S) => {
        const x = h - Z, b = S - Z;
        return new oo(h * j + S, x, b);
      })
    ), oe.forEach(
      (f, h) => f.forEach((w, S) => {
        w.neighbours = e(h - Z, S - Z), w.init();
      })
    ), Ie = n(0, 0);
  }
  function n(f, h) {
  ...
  function e(f, h) {
    return [-1, 0, 1].flatMap(
      (w) => [-1, 0, 1].map((S) => w === 0 && S === 0 ? null : n(f + w, h + S)).filter(Boolean)
    );
  }
  function u() {
...
const co = () => {
  let i = [], n = 0, s = null;
  function e(h, w) {
    console.debug(`url= ${h}`), i.push(async () => {
      try {
        const S = await fetch(h);
        console.debug(S);
        const x = await S.arrayBuffer(), b = new Uint32Array(x, 0, 1)[0], C = JSON.parse(new TextDecoder().decode(new Uint8Array(x, 4, b))), { vertexCount: P, indexCount: N, attributes: _ } = C;
        let p = 4 + b;
        ...
        console.error("Error loading buffer:", S);
      }
    });
  }
  function u(h, w, S, x, b) {
    const C = x.length, P = b.indexOf("Int") === 0, N = 1 << h.BYTES_PER_ELEMENT * 8, _ = P ? N * 0.5 : 0, p = 1 / N, g = new Float32Array(w * S);
  ...
  function d(h, w) {
    i.push(() => {
      new c.TextureLoader().load(
        ...
        (S) => {
          var x;
          S.minFilter = c.LinearMipMapLinearFilter, S.magFilter = c.LinearFilter, S.generateMipmaps = !0, S.anisotropy = ((x = t.renderer) == null ? void 0 : x.capabilities.getMaxAnisotropy()) || 1, S.flipY = !0, w && w(S), f();
        },
        void 0,
        (S) => console.error("Error loading texture:", S)
      );
    });
  }
  function v(h) {
...
function mo(i, n, s, e, u) {
  if (i === 0) return 0;
  if (i === 1) return 1;
  function d(h, w, S, x, b) {
    const C = 3 * (S - w), P = 3 * (x - S) - C;
    return (((b - w - C - P) * h + P) * h + C) * h + w;
  }
  function v(h, w, S, x = 1e-6) {
    let b = 0, C = 1, P = h;
    for (; b < C; ) {
      const N = d(P, 0, w, S, 1);
      if (Math.abs(N - h) < x)
...
const Eo = () => {
  ...
  function s() {
    ...
    for (let b = 0; b < x; b++) {
      const C = he.getRandomFreeTile();
      if (C) {
        const P = new Kt(L.length);
        P.currentTile = C, P.init(), P.updateTile(), L = [P, ...L];
      }
    }
  }
  function e() {
    var P;
    let x = null;
    const b = !!(t.errorBlock && t.errorBlock.errorLifeCycle >= t.errorBlockMaxLifeCycle), C = !!(L.length < t.maxFreeBlocksCount && Me);
    b ? ((P = t.errorBlock) == null || P.reset(!0), Pe.resetBlockFromLogicBlock(t.errorBlock), x = t.errorBlock, t.errorBlock = null) : C && (x = new Kt(L.length), z = x), x && (x.currentTile = Ie, x.init(), x.updateTile());
  }
  function u() {
  ...
  function d() {
    ...
    L.forEach((b, C) => {
      !b.hasBeenEvaluated && b.hasBeenSpawned && b.moveToNextTile(x, C * 0.2);
    });
  }
  function v() {
  ...
  function w(x) {
    ...
    le && q.setRestartAnimation(), he.preUpdate(x), z && z.update(x), L.forEach((C) => C.update(x)), he.update(x), h() && u();
  }
  function S() {
...
const Ne = 2 * Q, k = new c.Vector2(), et = new c.Vector2(), we = new c.Vector3(), an = new c.Vector3(), pt = new c.Quaternion(), rn = new c.Quaternion(), sn = new c.Color(), tt = new c.Color(), vt = new c.Color(), ye = new c.Color(), se = new c.Color(), nt = new c.Color(), Ae = new c.Object3D(), V = {
  ...
  function N(_) {
    ...
    for (let l = 0; l < Ne; l++) {
      ...
      y.showRatio > 0 && (a._blockRenderList[p++] = y), C(R, y, l), b(R, y), x(y, l), P(R, y);
    }
    h(), S(p);
...
const ze = {
  ...
  async function i() {
    ...
    ie.loadTexture(`${f}/matcap_gold.jpg`, (h) => {
      ...
      const { position: w, aoN: S, aoP: x, curveu: b, orient: C } = h.attributes;
      cn = w.array, fn = S.array, mn = x.array, dn = b.array, un = C.array, hn = w.count;
    });
  }
  function n() {
    s(), e(), u(), K && bn.add(K);
  ...
  function s() {
    ...
    v.index = ot.index, Object.entries(ot.attributes).forEach(([h, w]) => v.setAttribute(h, w)), pn = new Float32Array(hn * 3).map(() => Math.random() * 2 - 1), [
      ...
      v.setAttribute(h, new c.InstancedBufferAttribute(w, S));
    }), gt = v;
  }
  function e() {
...
`, Fo = () => {
  ...
  function e() {
    ...
    f.index = v.index, Object.keys(v.attributes).forEach((C) => {
      f.setAttribute(C, v.attributes[C]);
    }), f.instanceCount = 50;
    const h = new Float32Array(50 * 3), w = new Float32Array(50 * 3);
    for (let C = 0; C < 50; C++)
      h[C * 3] = 3 * (Math.random() * 2 - 1), h[C * 3 + 1] = Math.random() * 2 - 1, h[C * 3 + 2] = 0.5 + 0.5 * Math.random(), w[C * 3] = Math.random(), w[C * 3 + 1] = Math.random(), w[C * 3 + 2] = Math.random();
    ...
    n = new c.Mesh(f, S), n.renderOrder = 2, n.frustumCulled = !1, i.add(n);
  }
  function u(d) {
