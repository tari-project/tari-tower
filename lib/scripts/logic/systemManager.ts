...
const SystemManager = () => {
  function _spawnBlock() {
    ...
    if (blocks.length === properties.maxFreeBlocksCount && stateFlags.isFree)
      return;
  ...
  function _shouldPreventSpawn() {
    return (
      stateFlags.isFailResult ||
      stateFlags.isStopped ||
      blocks.length >= TOTAL_TILES ||
      (mainTile?.isOccupied &&
        !stateFlags.isSuccessResult &&
        !stateFlags.isReplayResult)
    );
  }
  ...
  function _spawnSingleBlock() {
    ...
    const needsErrorBlockReplacement = Boolean(
      properties.errorBlock &&
        properties.errorBlock.errorLifeCycle >=
          properties.errorBlockMaxLifeCycle
    );
    const canAddNewBlock = Boolean(
      blocks.length < properties.maxFreeBlocksCount &&
        stateStatus === AnimationStatus.FREE
    );
  ...
  function _calculatePaths() {
    ...
    const _isFree =
      cycleIndex % 2 === 0
        ? true
        : properties.activeBlocksCount < properties.maxFreeBlocksCount - 1;
  ...
  function _updateAnimationRatios(dt: number) {
    ...
    firstStartAnimationRatio = math.saturate(
      firstStartAnimationRatio + dt * (properties.showVisual ? 1 : 0)
    );

    animationSpeedRatio = Math.min(
      1,
      animationSpeedRatio + dt * (_isResult ? 1 : 0)
    );
    previousSuccessBlocksAnimationRatio = math.saturate(
      previousSuccessBlocksAnimationRatio - dt / 1.5
    );
  }
  ...
  function _checkCycleCompletion() {
    ...
    blocks.forEach((block) => {
      if (block.lifeCycle > 0) {
        isCycleComplete = Boolean(
          isCycleComplete && block.hasBeenEvaluated && block.hasAnimationEnded
        );
      } else {
