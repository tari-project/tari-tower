...
import { hasNotStarted, isFailResult, isFree, isReplayResult, isRestart, isResult, isResultAnimation, isStopped, isSuccessResult, PREVENT_CYCLE_STATES, resetCycleResults, result, stateManager, status, } from './stateManager';
...
let blocks = [];
let lastSpawnedBlock = null;
let cycleIndex = 0;
let animationSpeedRatio = 0;
let previousSuccessBlocksAnimationRatio = 0;
const SystemManager = () => {
    function _spawnBlock() {
        ...
        if (blocks.length === properties.maxFreeBlocksCount && isFree)
            return;
    ...
    function _updateAnimationRatios(dt) {
