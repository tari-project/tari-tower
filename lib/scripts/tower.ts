...
const TariTower = () => {
  ...
  function _handleResize(viewportWidth: number, viewportHeight: number) {
    ...
    properties.sharedUniforms.u_viewportResolution.value =
      properties.viewportResolution;
  ...
  function render(dt: number) {
    ...
    const aspect =
      (window.innerWidth - properties.cameraOffsetX) / window.innerHeight;
    const viewHeight = 10;
    const viewWidth = viewHeight * aspect;
