  ...
  constructor(n, s) {
    super(), s === void 0 && console.warn('THREE.OrbitControls: The second parameter "domElement" is now mandatory.'), s === document && console.error('THREE.OrbitControls: "document" should not be used as the target "domElement". Please use "renderer.domElement" instead.'), this.object = n, this.domElement = s, this.domElement.style.touchAction = "none", this.enabled = !0, this.target = new re(), this.minDistance = 0, this.maxDistance = 1 / 0, this.minZoom = 0, this.maxZoom = 1 / 0, this.minPolarAngle = Math.PI * 0.2, this.maxPolarAngle = Math.PI * 0.45, this.minAzimuthAngle = -1 / 0, this.maxAzimuthAngle = 1 / 0, this.enableDamping = !1, this.dampingFactor = 0.15, this.enableZoom = !1, this.zoomSpeed = 1, this.enableRotate = !0, this.rotateSpeed = 0.5, this.enablePan = !1, this.panSpeed = 1, this.screenSpacePanning = !0, this.autoRotate = !1, this.autoRotateSpeed = 2, this.keys = { LEFT: "ArrowLeft", UP: "ArrowUp", RIGHT: "ArrowRight", BOTTOM: "ArrowDown" }, this.mouseButtons = { LEFT: xe.ROTATE, MIDDLE: xe.DOLLY, RIGHT: xe.PAN }, this.touches = { ONE: ge.ROTATE, TWO: ge.DOLLY_PAN }, this.target0 = this.target.clone(), this.position0 = this.object.position.clone(), this.zoom0 = this.object.zoom, this.scale = 1, this.reset = function() {
      ...
      return function() {
        ...
        o.copy(Vt).sub(e.target), o.applyQuaternion(E), f.setFromVector3(o), e.autoRotate && d === u.NONE && B(y()), e.enableDamping ? (f.theta += v.theta * e.dampingFactor, f.phi += v.phi * e.dampingFactor) : (f.theta += v.theta, f.phi += v.phi);
        ...
        return f.radius *= jt, f.radius = Math.max(e.minDistance, Math.min(e.maxDistance, f.radius)), e.enableDamping === !0 ? e.target.addScaledVector(w, e.dampingFactor) : e.target.add(w), o.setFromSpherical(f), o.applyQuaternion(F), Vt.copy(e.target).add(o), e.object.lookAt(e.target), e.enableDamping === !0 ? (v.theta *= 1 - e.dampingFactor, v.phi *= 1 - e.dampingFactor, w.multiplyScalar(1 - e.dampingFactor)) : (v.set(0, 0, 0), w.set(0, 0, 0)), e.scale = e.scale / jt, C || U.distanceToSquared(e.object.position) > p || 8 * (1 - G.dot(e.object.quaternion)) > p ? (e.dispatchEvent(qt), U.copy(e.object.position), G.copy(e.object.quaternion), C = !1, !0) : !1;
      };
    }();
    const e = this, u = {
      NONE: -1,
    ...
    let d = u.NONE;
    const p = 1e-6, f = new Xt(), v = new Xt(), w = new re();
    let C = !1;
    const g = new $(), b = new $(), S = new $(), P = new $(), N = new $(), _ = new $(), h = new $(), x = new $(), m = new $(), r = [], l = {};
    function y() {
    ...
    function Cn(o) {
      b.set(o.clientX, o.clientY), S.subVectors(b, g).multiplyScalar(e.rotateSpeed);
      const E = e.domElement;
      B(2 * Math.PI * S.x / E.clientHeight), H(2 * Math.PI * S.y / E.clientHeight), g.copy(b), e.update();
    }
    function En(o) {
      x.set(o.clientX, o.clientY), m.subVectors(x, h), m.y > 0 ? Le(R()) : m.y < 0 && de(R()), h.copy(x), e.update();
    }
    function Rn(o) {
    ...
    function zt(o) {
      ...
      S.subVectors(b, g).multiplyScalar(e.rotateSpeed);
      const E = e.domElement;
      B(2 * Math.PI * S.x / E.clientHeight), H(2 * Math.PI * S.y / E.clientHeight), g.copy(b);
    }
    function It(o) {
      if (r.length === 1)
    ...
    function Ft(o) {
      ...
      x.set(0, G), m.set(0, Math.pow(x.y / h.y, e.zoomSpeed)), Le(m.y), h.copy(x);
    }
    function Ln(o) {
...
const ao = () => {
  function i() {
    oe = Array.from(
      { length: j },
      (f, v) => Array.from({ length: j }, (w, C) => {
        const g = v - Z, b = C - Z;
        return new oo(v * j + C, g, b);
      })
    ), oe.forEach(
      (f, v) => f.forEach((w, C) => {
        w.neighbours = e(v - Z, C - Z), w.init();
      })
    ), Ie = n(0, 0);
  }
  function n(f, v) {
    var w;
    return ((w = oe[f + Z]) == null ? void 0 : w[v + Z]) || null;
  }
  function s() {
    const f = oe.flat().filter((v) => !v.isOccupied);
    return f.length ? f[Math.floor(Math.random() * f.length)] : null;
  }
  function e(f, v) {
    return [-1, 0, 1].flatMap(
      (w) => [-1, 0, 1].map((C) => w === 0 && C === 0 ? null : n(f + w, v + C)).filter(Boolean)
    );
  }
  function u() {
  ...
  function d(f) {
    oe.flat().forEach((v) => v.preUpdate(f));
  }
  function p(f) {
    oe.flat().forEach((v) => v.update(f));
  }
  return {
    init: i,
    getTile: n,
...
}, he = ao(), xn = new c.Vector2(), gn = new c.Vector2(), ro = {
  u_time: { value: 0 },
  u_deltaTime: { value: 1 },
  u_resolution: { value: xn },
  u_viewportResolution: { value: gn },
...
const co = () => {
  ...
  function e(v, w) {
    i.push(async () => {
      try {
        const g = await (await fetch(v)).arrayBuffer(), b = new Uint32Array(g, 0, 1)[0], S = JSON.parse(new TextDecoder().decode(new Uint8Array(g, 4, b))), { vertexCount: P, indexCount: N, attributes: _ } = S;
        let h = 4 + b;
        const x = new c.BufferGeometry(), m = {};
        _.forEach((r) => {
          const { id: l, componentSize: y, storageType: R, needsPack: B, packedComponents: H } = r, X = l === "indices" ? N : P, Y = window[R], De = new Y(g, h, X * y), Le = Y.BYTES_PER_ELEMENT;
          let de;
          B ? de = u(De, X, y, H, R) : (m[l] = h, de = De), l === "indices" ? x.setIndex(new c.BufferAttribute(de, 1)) : x.setAttribute(l, new c.BufferAttribute(de, y)), h += X * y * Le;
        }), w && w(x), f();
      } catch (C) {
  ...
  function u(v, w, C, g, b) {
    const S = g.length, P = b.indexOf("Int") === 0, N = 1 << v.BYTES_PER_ELEMENT * 8, _ = P ? N * 0.5 : 0, h = 1 / N, x = new Float32Array(w * C);
    for (let m = 0, r = 0; m < w; m++)
      for (let l = 0; l < S; l++) {
        const { delta: y, from: R } = g[l];
        x[r] = (v[r] + _) * h * y + R, r++;
      }
    return x;
  }
  function d(v, w) {
    i.push(() => {
      new c.TextureLoader().load(
        ...
        (C) => {
          var g;
          C.minFilter = c.LinearMipMapLinearFilter, C.magFilter = c.LinearFilter, C.generateMipmaps = !0, C.anisotropy = ((g = t.renderer) == null ? void 0 : g.capabilities.getMaxAnisotropy()) || 1, C.flipY = !0, w && w(C), f();
        },
        void 0,
...
class uo {
  ...
  fit(n, s, e, u, d, p) {
    return n = this.cUnMix(s, e, n), p && (n = p(n)), u + n * (d - u);
  }
}
const A = new uo(), fo = () => {
  function i(d) {
  ...
  function e(d, p = 1.70158) {
    return --d * d * ((p + 1) * d + p) + 1;
  }
  function u(d) {
    const p = 2.5949095;
    return (d *= 2) < 1 ? 0.5 * d * d * ((p + 1) * d - p) : 0.5 * ((d -= 2) * d * ((p + 1) * d + p) + 2);
  }
  return {
    quartInOut: i,
    sineOut: n,
...
function mo(i, n, s, e, u) {
  if (i === 0) return 0;
  if (i === 1) return 1;
  function d(v, w, C, g, b) {
    const S = 3 * (C - w), P = 3 * (g - C) - S;
    return (((b - w - S - P) * v + P) * v + S) * v + w;
  }
  function p(v, w, C, g = 1e-6) {
    let b = 0, S = 1, P = v;
    for (; b < S; ) {
      const N = d(P, 0, w, C, 1);
      if (Math.abs(N - v) < g)
...
}, We = po(), Ge = new ae(), vo = new ae(), xo = new ae(), go = new ae(), wt = new ae(), yt = new ae(), Tt = new ae(), _o = new ae(), wo = new ae();
...
const bo = () => {
  ...
  function f() {
    p({ status: D.NOT_STARTED, result: M.NONE });
  }
  function v() {
    p({ status: D.STARTED });
  }
  ...
  function b() {
    p({ status: D.RESULT, result: M.STOP });
  }
  function S(l = !1) {
    const y = l && Fe ? M.REPLAY : M.COMPLETED;
    p({ status: D.RESULT, result: y, animationStyle: ue.ONE });
  }
  function P(l = !1) {
    const y = l && Fe ? M.REPLAY : M.COMPLETED;
    p({ status: D.RESULT, result: y, animationStyle: ue.TWO });
  }
  function N(l = !1) {
    const y = l && Fe ? M.REPLAY : M.COMPLETED;
    p({ status: D.RESULT, result: y, animationStyle: ue.THREE });
  }
  function _() {
    p({ status: D.RESULT, result: M.FAILED });
  }
  function h() {
    p({ status: D.RESULT_ANIMATION });
  }
  function x() {
    p({ status: D.RESTART_ANIMATION });
  }
  function m() {
    dt.push(() => {
      s(D.RESTART) && go.dispatch();
    });
  }
  function r() {
    n();
...
const Co = () => {
  function i() {
    Ge.add((f, v, w) => {
      f === D.RESULT && (v === M.COMPLETED || v === M.REPLAY) && n(w);
    });
  }
  function n(f) {
    Ke = f;
...
const Eo = () => {
  ...
  function s() {
    let g = Q - t.activeBlocksCount;
    t.errorBlock && (t.errorBlock.currentTile && (t.errorBlock.currentTile.isOccupied = !1), g += 1);
    for (let b = 0; b < g; b++) {
  ...
  function e() {
    ...
    b ? ((P = t.errorBlock) == null || P.reset(!0), Pe.resetBlockFromLogicBlock(t.errorBlock), g = t.errorBlock, t.errorBlock = null) : S && (g = new Kt(L.length), z = g), g && (g.currentTile = Ie, g.init(), g.updateTile());
  }
  function u() {
    q.updateAfterCycle(), !yo.includes(W) && (z && (L = [z, ...L], z = null), t.activeBlocksCount = L.length, !(at || rt) && (L.forEach((g) => g.resetAfterCycle()), xo.dispatch(), ht++, i(), d()));
  }
  function d() {
    z != null && z.hasBeenSpawned && z.moveToNextTile(Me, 0);
    const g = ht % 2 === 0 ? !0 : t.activeBlocksCount < t.maxFreeBlocksCount - 1;
    L.forEach((b, S) => {
      !b.hasBeenEvaluated && b.hasBeenSpawned && b.moveToNextTile(g, S * 0.2);
    });
  }
  function p() {
    L.forEach((b) => b.reset()), Pe.reset(), he.reset(), L = [], z = null, ht = 0;
    const g = To.includes(I);
    q.reset(), u(), g && q.setStart(), yt.remove(() => {
      q.setRestart(), u(), Ve = 1;
    }), Tt.remove(() => {
      q.setRestart(), p();
    }), wt.remove(() => {
  ...
  function f(g) {
    Et = A.saturate(Et + g * (t.showVisual ? 1 : 0)), Ve = A.saturate(Ve - g / 1.5);
  }
  function v() {
    let g = !0;
    return z && (g = !!(g && z.hasBeenSpawned)), L.forEach((b) => {
      b.lifeCycle > 0 ? g = !!(g && b.hasBeenEvaluated && b.hasAnimationEnded) : g = g && b.spawnAnimationRatio === 1;
    }), g || le || at || rt;
  }
  function w(g) {
    if (f(g), tn.update(g), Jt.update(g), en.update(g), Fe) {
      u();
      return;
    }
    if (wn) {
    ...
    le && q.setRestartAnimation(), he.preUpdate(g), z && z.update(g), L.forEach((S) => S.update(g)), he.update(g), v() && u();
  }
  function C() {
...
const Ne = 2 * Q, k = new c.Vector2(), et = new c.Vector2(), we = new c.Vector3(), an = new c.Vector3(), pt = new c.Quaternion(), rn = new c.Quaternion(), sn = new c.Color(), tt = new c.Color(), vt = new c.Color(), ye = new c.Color(), se = new c.Color(), nt = new c.Color(), Ae = new c.Object3D(), V = {
  ...
  async function i() {
    ...
    const h = qe + "/models", x = qe + "/textures";
    ie.loadBuf(`${h}/BASE.buf`, (m) => {
      n(m);
    }), ie.loadBuf(`${h}/BOX.buf`, (m) => {
      s(m);
    }), ie.loadBuf(`${h}/LOSE_ANIMATION.buf`, (m) => {
  ...
  function n(_) {
    ...
    a._baseMesh = new c.Mesh(_, x), a._baseMesh.receiveShadow = a._baseMesh.castShadow = !0, a._baseMesh.frustumCulled = !1, a._baseMesh.customDepthMaterial = new c.ShaderMaterial({
      vertexShader: Je,
      fragmentShader: on,
  ...
  function s(_) {
    const h = new c.InstancedBufferGeometry();
    h.index = _.index, Object.keys(_.attributes).forEach((r) => {
      h.setAttribute(r, _.attributes[r]);
    }), h.instanceCount = Ne;
    const x = (r, l) => {
      const y = new Float32Array(Ne * l);
      return h.setAttribute(r, new c.InstancedBufferAttribute(y, l, l !== 4, 1).setUsage(c.DynamicDrawUsage)), y;
    };
    a._instancePosArray = x("instancePos", 3), a._instanceOrientArray = x("instanceOrient", 4), a._instanceShowRatioArray = x("instanceShowRatio", 1), a._instanceSpinPivotArray = x("instanceSpinPivot", 3), a._instanceSpinOrientArray = x("instanceSpinOrient", 4), a._instanceColorArray = x("instanceColor", 3), a._instanceIsActiveArray = x("instanceIsActive", 1), a._instanceNextDirectionArray = x("instanceNextDirection", 2);
    ...
    a._blocksMesh = new c.Mesh(h, m), a._blocksMesh.frustumCulled = !1, a._blocksMesh.castShadow = a._blocksMesh.receiveShadow = !0, a._blocksMesh.customDepthMaterial = new c.ShaderMaterial({
      uniforms: { ...V },
      vertexShader: Je,
      fragmentShader: on,
      defines: { IS_DEPTH: !0 }
  ...
  function e() {
    var h, x;
    a.directLight = new c.DirectionalLight(16777215, 1), a.directLight.castShadow = !0, a.directLight.shadow.camera.near = t.lightCameraNear, a.directLight.shadow.camera.far = t.lightCameraFar, a.directLight.shadow.camera.right = t.lightCameraSize, a.directLight.shadow.camera.left = -t.lightCameraSize, a.directLight.shadow.camera.top = t.lightCameraSize, a.directLight.shadow.camera.bottom = -t.lightCameraSize, a.directLight.shadow.bias = t.lightCameraBias, a.directLight.shadow.mapSize.width = 768, a.directLight.shadow.mapSize.height = 768, (h = t.scene) == null || h.add(a.directLight), (x = t.scene) == null || x.add(a.directLight.target), a.isShadowCameraHelperVisible = !0, a.shadowCameraHelper = new c.CameraHelper(a.directLight.shadow.camera), wo.add(() => {
      var m, r;
  ...
  function u() {
    oe.forEach((_, h) => {
      _.forEach((x, m) => {
        var l, y;
        const r = h * j + m;
        x.loseAnimationPositionArray = new Float32Array(a.animationTotalFrames * 3), x.loseAnimationOrientArray = new Float32Array(a.animationTotalFrames * 4);
        for (let R = 0; R < a.animationTotalFrames; R++) {
          const B = (R * Q + r) * 3, H = (R * Q + r) * 4;
          x.loseAnimationPositionArray.set(((l = a.heroLoseAnimationPositionArray) == null ? void 0 : l.subarray(B, B + 3)) || [], R * 3), x.loseAnimationOrientArray.set(((y = a.heroLoseAnimationOrientArray) == null ? void 0 : y.subarray(H, H + 4)) || [], R * 4);
        }
      });
    });
  }
  ...
  function f(_) {
    ...
    for (let m = 0; m < Ne; m++) {
      ...
      if (l && (r != null && r.isErrorBlock)) {
        ...
        r.errorFallAnimationTime > 0 && (R = A.saturate(0.5 * (1 - Math.cos(14 * r.errorFallAnimationTime)))), nt.lerpColors(y, vt, R), (h = a._instanceColorArray) == null || h.set([nt.r, nt.g, nt.b], m * 3);
      } else
        (x = a._instanceColorArray) == null || x.set([y.r, y.g, y.b], m * 3);
  ...
  function v() {
    oe.forEach((_) => {
      _.forEach((h) => {
        const x = h.id % j + 1, r = ((Math.floor(h.id / j) + 1) * ne + x) * 4;
        let l = 0.5 * Ce * A.fit(Re, 0, 0.1, 1, 0);
        l += (pe > 0 ? 1 : 0) * A.fit(Xe, 0, 0.1, 1, 0), l += je * A.fit(Ze, 0, 0.1, 1, 0), l = Math.min(1, l), a.infoTexture && (a.infoTexture.image.data[r] = h.activeRatio * (1 - l), a.infoTexture.image.data[r + 1] = h.isOccupied || h.willBeOccupied ? 1 : 0, a.infoTexture.image.data[r + 2] = h.isMain ? 1 : 0, a.infoTexture.image.data[r + 3] = h.isBorder ? 1 : 0);
      });
  ...
  function w() {
    var _, h;
    if (z) {
      const x = a._blockList[z.id];
      z.currentTile && x.boardPos.set((_ = z.currentTile) == null ? void 0 : _.row, (h = z.currentTile) == null ? void 0 : h.col), x.showRatio = Te(A.saturate(z.spawnAnimationRatioUnclamped));
    }
    L.forEach((x) => {
      var r, l, y, R;
      const m = a._blockList[x.id];
      m && (m.showRatio = Te(A.saturate(x.spawnAnimationRatioUnclamped)), x.currentTile && m.boardPos.set((r = x.currentTile) == null ? void 0 : r.row, (l = x.currentTile) == null ? void 0 : l.col), x.targetTile && m.boardDir.set(x.targetTile.row - (((y = x.currentTile) == null ? void 0 : y.row) || 0), x.targetTile.col - (((R = x.currentTile) == null ? void 0 : R.col) || 0)), m.animation = x.hasAnimationEnded ? 0 : x.easedAnimationRatio);
    });
  }
  function C(_) {
    var x, m;
    for (let r = 0; r < _; r++) {
      ...
      l.pos.toArray(a._instancePosArray || [], r * 3), l.orient.toArray(a._instanceOrientArray || [], r * 4), a._instanceShowRatioArray && (a._instanceShowRatioArray[r] = Oe.quartInOut(l.showRatio)), l.spinPivot.toArray(a._instanceSpinPivotArray || [], r * 3), l.spinOrient.toArray(a._instanceSpinOrientArray || [], r * 4), (x = a._instanceNextDirectionArray) == null || x.set([l.boardDir.x, l.boardDir.y], r * 2);
    }
    const h = (m = a._blocksMesh) == null ? void 0 : m.geometry;
    if (h) {
      for (const r in h.attributes) {
        const l = h.attributes[r];
  ...
  function g(_, h) {
    if (I === M.STOP && h >= Q) {
      const x = h - Q, m = x % j - Z, r = Math.floor(x / j) - Z, l = he.getTile(r, m);
      if (!l.isOccupied) {
  ...
  function b(_, h) {
    if (_ && _.isErrorBlock && _.errorLifeCycle >= t.errorBlockMaxLifeCycle - 1) {
      const x = _.currentTile, m = _.errorFallAnimationTime;
      h.boardPos.set(x.row, x.col), k.set(x.row, x.col).normalize(), Math.abs(k.x) > Math.abs(k.y) ? k.set(Math.sign(k.x), 0) : k.set(0, Math.sign(k.y)), h.boardDir.set(k.x, k.y), h.animation = A.fit(m, 0, 1, 0, 1, Oe.sineOut), h.animation += Math.max(0, m - 0.8), h.update(t.deltaTime), h.addsFallAnimation(Math.max(0, m - 0.8));
    }
  }
  function S(_, h, x) {
    if (I === M.FAILED) {
      if (_) {
        ...
        if (pe > 0) {
          ...
          we.fromArray(m.loseAnimationPositionArray, r * 3), an.fromArray(m.loseAnimationPositionArray, l * 3), we.lerp(an, y), we.y *= 0.5, h.pos.set(we.z, we.y, -we.x), pt.fromArray(m.loseAnimationOrientArray, r * 4), rn.fromArray(m.loseAnimationOrientArray, l * 4), pt.slerp(rn, y), h.orient.copy(pt);
        }
        if (Se > 0) {
          const r = A.fit(Se, 0, 1, 0, 1, Oe.sineOut);
          if (k.set(m.row, m.col), k.normalize(), k.multiplyScalar(0.1 * r), h.pos.x += k.x, h.pos.z -= k.y, Se < 1) {
            const l = r * A.fit(Se, 0.5, 0.8, 1, 0);
            k.set(_.randomVector.x, _.randomVector.y), k.normalize(), k.multiplyScalar(l), et.set(0, 0), et.addScaledVector(k, 0.08 * l * Math.sin(l * 80)), h.pos.x += et.x, h.pos.z += et.y;
          }
        }
      }
      if (x >= Q) {
        const m = x - Q, r = m % j - Z, l = Math.floor(m / j) - Z, y = he.getTile(l, r), R = A.saturate(st - y.randomDelay);
        y.isOccupied || (y.activeRatio = R), h.showRatio = Te(R), h.boardPos.set(l, r);
      }
    }
  }
  function P(_, h) {
    if ((I === M.COMPLETED || I === M.REPLAY) && _) {
      ...
      l = A.fit(r, 0.7, 1, l, 0, (y) => Math.pow(y, 5)), h.pos.y += Ye * l;
    }
  }
  function N(_) {
    w(), f(_);
    ...
    for (let l = 0; l < Ne; l++) {
      ...
      y.showRatio > 0 && (a._blockRenderList[h++] = y), S(R, y, l), b(R, y), g(y, l), P(R, y);
    }
    v(), C(h);
    const x = Math.min(1, Ze + Xe + Re), m = Oe.backOut(x, 3), r = 1 - Te(Et);
...
`, bn = new c.Object3D();
let K = null, xt = null, gt = null, cn = null, un = null, dn = null, fn = null, mn = null, hn = 0, Be = 0, _t, ot, pn;
const ze = {
  u_time: { value: 0 },
  u_ratio: { value: 0 },
  ...
  async function i() {
    const p = qe + "/models", f = qe + "/textures";
    ie.loadTexture(`${f}/matcap_gold.jpg`, (v) => {
      _t = v, _t.needsUpdate = !0;
    }), ie.loadBuf(`${p}/COIN.buf`, (v) => {
      ot = v;
    }), ie.loadBuf(`${p}/COIN_PLACEMENT.buf`, (v) => {
      const { position: w, aoN: C, aoP: g, curveu: b, orient: S } = v.attributes;
      cn = w.array, fn = C.array, mn = g.array, dn = b.array, un = S.array, hn = w.count;
    });
  }
  function n() {
    s(), e(), u(), K && bn.add(K);
  ...
  function s() {
    ...
    p.index = ot.index, Object.entries(ot.attributes).forEach(([v, w]) => p.setAttribute(v, w)), pn = new Float32Array(hn * 3).map(() => Math.random() * 2 - 1), [
      ...
      p.setAttribute(v, new c.InstancedBufferAttribute(w, C));
    }), xt = p;
  }
  function e() {
    gt = new c.ShaderMaterial({
      uniforms: {
        ...V,
        ...t.sharedUniforms,
        ...ze,
  ...
  function u() {
    xt && gt && (K = new c.Mesh(xt, gt), K.frustumCulled = !1, K.castShadow = !0, K.receiveShadow = !0, K.customDepthMaterial = new c.ShaderMaterial({
      uniforms: { ...ze },
      vertexShader: ln,
      fragmentShader: Lo,
      defines: { IS_DEPTH: !0 }
  ...
  function d(p) {
    const f = Ee === 0;
    Be = f ? Ue : Ee, ze.u_ratio.value = Be, ze.u_time.value += p, ze.u_isFloating.value = f ? 1 : 0, K && (K.rotation.y = 0 * Be, K.visible = Be > 0 && Be < 1);
  }
  return {
    preload: i,
    init: n,
...
`, Fo = () => {
  ...
  function s() {
    ...
    const d = new c.ShaderMaterial({
      uniforms: Object.assign(
        {
          ...
          u_bgColor1: (v = t.sharedUniforms) == null ? void 0 : v.u_bgColor1,
      ...
      vertexShader: No,
      fragmentShader: Bo
    }), p = new c.Mesh(new c.PlaneGeometry(2, 2), d);
    p.renderOrder = 1, i.add(p), e();
  }
  function e() {
    var g, b;
    const p = new c.PlaneGeometry(1, 1), f = new c.InstancedBufferGeometry();
    f.index = p.index, Object.keys(p.attributes).forEach((S) => {
      f.setAttribute(S, p.attributes[S]);
    }), f.instanceCount = 50;
    const v = new Float32Array(50 * 3), w = new Float32Array(50 * 3);
    for (let S = 0; S < 50; S++)
      v[S * 3] = 3 * (Math.random() * 2 - 1), v[S * 3 + 1] = Math.random() * 2 - 1, v[S * 3 + 2] = 0.5 + 0.5 * Math.random(), w[S * 3] = Math.random(), w[S * 3 + 1] = Math.random(), w[S * 3 + 2] = Math.random();
    f.setAttribute("a_instancePosition", new c.InstancedBufferAttribute(v, 3)), f.setAttribute("a_instanceRandom", new c.InstancedBufferAttribute(w, 3));
    const C = new c.ShaderMaterial({
      vertexShader: zo,
      fragmentShader: Io,
      uniforms: {
        u_time: ((g = t.sharedUniforms) == null ? void 0 : g.u_time) || { value: t.time },
...
function Ho(i) {
  ...
  const p = t.camera;
  (f = t.orbitControls) == null || f.update(), (v = t.orbitCamera) == null || v.updateMatrix(), p && ((w = t.orbitCamera) == null || w.matrix.decompose(p.position, p.quaternion, p.scale), p.matrix.compose(p.position, p.quaternion, p.scale)), Pe.update(i), Pt.update(i), Rt.update(i), t.camera && t.scene && t.renderer && t.renderer.render(t == null ? void 0 : t.scene, t.camera);
}
function An() {
  var s;
...
function Vo(i, n) {
  ...
  if (s * e > ce.MAX_PIXEL_COUNT) {
    const p = s / e;
    e = Math.sqrt(ce.MAX_PIXEL_COUNT / p), s = Math.ceil(e * p), e = Math.ceil(e);
  }
...
function Zo(i) {
  ...
  n == null || n.remove(), (e = (s = t.renderer) == null ? void 0 : s.domElement) == null || e.remove(), (d = (u = t.renderer) == null ? void 0 : u.state) == null || d.reset(), (p = t.renderer) == null || p.resetState(), t.canvas = void 0, t.orbitTarget = void 0, (f = t.renderer) == null || f.dispose(), t.renderer = void 0, lo();
}
async function $o(i) {
  ...
  const s = {
    ...
    [M.REPLAY]: Ct,
    [M.STOP]: St
  }, e = W === "not-started", u = I !== null, d = 1e3 * 3.5, p = (s[I] || 1) * 1e3 + d, f = St * 1e3 * 2, v = u ? p : d, w = u ? f + p : f;
  e || setTimeout(() => Xo("stop"), w), setTimeout(() => Zo(i), e ? v : v + w);
}
function Xo(i, n = !1) {
  q.set(i, n);
}
